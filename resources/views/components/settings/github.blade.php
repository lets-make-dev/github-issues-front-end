...
<div x-data="{
    ...
    disconnectRepo(repoId) {
        ...
        $wire.disconnectRepository(repoId).then(() => {
    ...
    $watch('repositories', value => { repositories = value; filterRepositories(); });
    filterRepositories();
...
<h3 class="text-2xl font-semibold mb-4">GitHub Integration</h3>
    ...
    <div x-show="showAccounts || connectedRepositories.length == 0">
        ...
        <div x-show="selectedAccountId && repositories.length > 0" class="mb-6">
            <h3 class="text-xl font-semibold mb-4">Add Repository To Projects <span x-text="filteredRepositories.length"></span></h3>
            <div class="relative" x-ref="dropdownContainer">
                <div class="flex">
                    <input
                        ...
                        @click="repoOpen = true"
                        type="text"
                        class="w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-500 focus:border-gray-500"
                        placeholder="Search repositories...">
                    <button @click="repoOpen = !repoOpen" type="button"
                            class="ml-2 bg-white border border-gray-300 rounded-md shadow-sm px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-500 focus:border-gray-500">
                ...
                <div x-show="repoOpen"
                    ...
                    <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label"
                        ...
                        <li x-show="filteredRepositories.length === 0" class="text-gray-500 cursor-default select-none relative py-2 pl-3 pr-9">
    ...
    <div class="mb-6" x-show="connectedRepositories.length > 0">
        ...
        <ul class="space-y-2">
            <template x-for="repo in connectedRepositories" :key="repo.id">
                <li class="repo-item flex justify-between items-center p-2 bg-white rounded shadow"
                    ...
                    <template x-if="!processingRepos[repo.id] && !deletingRepos[repo.id]">
                        <button
                            ...
                            class="text-red-600 hover:text-red-800">Disconnect</button>
                    </template>
