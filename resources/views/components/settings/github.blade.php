...
<div x-data="{
    ...
    disconnectRepo(repoId) {
        ...
        $wire.disconnectRepository(repoId).then(() => {
            ...
            }, 2000); // Increased to 2 seconds to ensure animation completes
        }).catch(() => {
            this.processingRepos[repoId] = false;
        });
    }
}" x-init="$watch('$wire.connectedRepositories', value => connectedRepositories = value)">
<h3 class="text-2xl font-semibold mb-4">GitHub Integration</h3>
    <div x-show="!showAccounts" class="py-4">
                ...
                <div x-show="repoOpen" @click.away="repoOpen = false"
                    ...
                    <ul tabindex="-1" role="listbox" aria-labelledby="listbox-label"
                        ...
                        <template x-for="repo in repositories" :key="repo">
                            <li @click="$wire.connectRepository(repo); selectedRepo = null; repoOpen = false"
                                class="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9 hover:bg-gray-100"
                                role="option">
